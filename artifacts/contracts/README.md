...
- It is **not** the same as the per-project emitted artifacts produced by workflows (for example `AUTH.contract.yaml`, `SYSTEM_HLD.contract.yaml`). Those workflow outputs land in the consumer repo under paths defined by the emitting skill — typically inside `artifacts/` or a documented subfolder.
