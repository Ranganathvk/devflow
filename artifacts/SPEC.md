...
3. **Human code ownership** — Humans own merge quality (tests, PR, optional `/review` or a third-party review skill). `/implement` marks tasks done.
...
- `/to-spec` — when a requirements / PRD document already exists
- `/grillme` — interview to refine SPEC (and optional HLD/LLD)
...
- `/implement` *(next queued task)* or `/implement <FEATURE>` *(lite or next for feature)* or `/implement <TASK_ID>`
- `/review` *(optional)* → `/security-review` *(optional)* → `/learn` *(optional)*

**Illustration (standard):** `/design AUTH` → `/tdd AUTH` → `/tasksplit AUTH` → `/implement` (repeat). Optional `/review` or a third-party review skill.

**Illustration (lite):** `/design AUTH` with `needs_tasks: false` → `/implement AUTH`. Optional review.
...
- **Attached repos** use a **configurable** top-level **context directory** for **human-owned durable workflow state** (default **`artifacts/`**; at minimum `artifacts/SPEC.md`; optional companions such as `artifacts/PROJECT_STATE.md`).
- **Override:** set environment variable **`DEVFLOW_CONTEXT_DIR`**, or repo-root **`devflow.context.yaml`** (`context_dir: your-folder`). Installers seed the manifest when missing.
- The **installer** creates the context directory when missing and may seed starter files; skills default to paths under **`artifacts/`** — substitute your resolved name when it differs.
- **Per-project emitted** artifacts (for example `*.contract.yaml`, feature design stubs) use locations **defined by the emitting skills** — often under the context directory or a documented subfolder.

### Durable SPEC path (v1, agreed)

- The **only authoritative product specification** in an attached repo (including this framework repo when dogfooding) is **`<context-dir>/SPEC.md`** (default **`artifacts/SPEC.md`**). Skills that require “the SPEC” **read and update this path** unless a skill explicitly documents a rare, scoped exception.  
- **No root-level `spec.md` (agreed):** This framework repo **does not** keep a second spec-shaped file at the repository root. `**README.md`** is the **primary** entry for humans and **must** link to `**artifacts/SPEC.md`** for the full product specification. Any historical root `**spec.md**` is **removed** once that link exists (same rule for other attached repos that adopt this framework: avoid duplicate root specs).
...
*Version:* framework SPEC; DevOps-agent draft superseded (**A — Framework only**); authoritative `**core/`** tree with agent-local mirrors **derived** via installer/sync (**A** ×3); consumer `**artifacts/`** anchor (**A**); durable SPEC **only** `**artifacts/SPEC.md`**; **no root `spec.md`** — `**README.md**` links here (**A**, 2026-05-13).