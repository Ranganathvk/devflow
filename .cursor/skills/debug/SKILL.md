...
3. **Load** `AI_CONTEXT/<FEATURE>_C<n>_REVIEW.contract.yaml`. If missing and human did not supply a defect list → **stop**; suggest `/review` first.
...
7. **Update review artifact:** Patch `AI_CONTEXT/<FEATURE>_C<n>_REVIEW.contract.yaml`:
...
8. **Append** a short **“Debug log”** section to `AI_CONTEXT/<FEATURE>_C<n>_REVIEW.md` (≤ ~40 lines): what changed, commands run, results.
