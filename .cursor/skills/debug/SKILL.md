...
- **Required:** `artifacts/<FEATURE>_C<n>_REVIEW.contract.yaml` for that task (create via `/review` first **or** human provides equivalent pasted findings — if missing, run `/review` first unless human explicitly waived with a pasted defect list).
- **Required:** `artifacts/SPEC.md` — **Implementation rules** and **Design principles** (minimum).
- **Optional (legacy):** `artifacts/<FEATURE>_PLAN.contract.yaml` — task row when deprecated plan queue used.
- **Optional:** `artifacts/<FEATURE>_TASKS.contract.yaml` — task row for scope.
...
3. **Load** `artifacts/<FEATURE>_C<n>_REVIEW.contract.yaml`. If missing and human did not supply a defect list → **stop**; suggest `/review` first.
...
7. **Update review artifact:** Patch `artifacts/<FEATURE>_C<n>_REVIEW.contract.yaml`:
...
8. **Append** a short **“Debug log”** section to `artifacts/<FEATURE>_C<n>_REVIEW.md` (≤ ~40 lines): what changed, commands run, results.
...
| `artifacts/<FEATURE>_C<n>_REVIEW.contract.yaml` | Patched | Resolution + readiness flags |
| `artifacts/<FEATURE>_C<n>_REVIEW.md` | Appended section | Debug log |
