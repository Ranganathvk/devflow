...
All workflow paths use **`artifacts/`**. Resolve per `core/AGENTS.md` (`DEVFLOW_CONTEXT_DIR` → `devflow.context.yaml` → default `artifacts`).

## Inputs

- **Required:** Git working tree (repo root).
- **Required:** Change set — resolve in workflow step 1. Never treat chat as the diff.
- **Optional:** `artifacts/SPEC.md` — only **Human code ownership** / compliance notes if present.
...
4. **Write** `artifacts/SECURITY_REVIEW.md` (replace, ≤ **~120** lines) and `artifacts/SECURITY_REVIEW.contract.yaml`.
...
| Path | Change | Notes |
|------|--------|-------|
| `artifacts/SECURITY_REVIEW.md` | Created or replaced | Human report |
| `artifacts/SECURITY_REVIEW.contract.yaml` | Created or replaced | Small YAML |

No other files written or edited. Do **not** change task `status`.

## `artifacts/SECURITY_REVIEW.md` shape
...
## `artifacts/SECURITY_REVIEW.contract.yaml` shape
