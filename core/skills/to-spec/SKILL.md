...
description: >-
  ...
  no interview. Writes {context_dir}/SPEC.md at high-level product intent.
...
spec at `{context_dir}/SPEC.md`. Do **not** interview the user — synthesize what
...
|-----------------------|------------|
| Conversation → issue tracker | Document → `{context_dir}/SPEC.md` |
...
- **Optional:** Existing `{context_dir}/SPEC.md` (merge carefully — see below),
  `{context_dir}/PROJECT_STATE.md`, domain glossary / ADRs in-repo.
...
`{context_dir}/SPEC.md` is **high-level product intent only** — same bar as
...
5. **Write `{context_dir}/SPEC.md`** using the **output template** below.
...
| Path | Change | Notes |
|------|--------|-------|
| `{context_dir}/SPEC.md` | Created or replaced/merged | Product-intent SPEC only |
