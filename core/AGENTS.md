...
| 2 | `{context_dir}/SPEC.md` | What we are building — **high-level product intent only** (goals, boundary, requirements); not technical or low-level design (that lives in HLD/LLD) |
...
Workflow files (SPEC, PROJECT_STATE, contracts, design docs) live under a **context directory** at the repo root. Canonical skills and contracts use the token **`{context_dir}/`** in paths (not a hardcoded folder name).

Resolve the directory name **before** reading or writing any `{context_dir}/…` path:

1. Environment variable **`DEVFLOW_CONTEXT_DIR`**
2. Repo-root **`devflow.context.yaml`** → `context_dir`
3. Default: **`artifacts`**

Example: `{context_dir}/SPEC.md` → `artifacts/SPEC.md` when the default applies, or `my-context/SPEC.md` when configured.
...
1. Read this harness.
2. Read `{context_dir}/SPEC.md` for goals, constraints, and success criteria.
3. Read `{context_dir}/PROJECT_STATE.md` (if present) for decisions, active work, and blockers.
4. If the user invoked a `/<command>`, read the matching `SKILL.md` and follow it literally.
5. Execute the task; update `{context_dir}/PROJECT_STATE.md` when completion or material facts change.
6. Only then propose follow-ups — prefer updating files over long summaries.

## Context rotation

- **Rotate out.** Once a slice of work is done, capture outcomes, decisions, and next steps in `{context_dir}/PROJECT_STATE.md`; avoid relying on thread history for facts.
...
- **SPEC vs state.** `{context_dir}/SPEC.md` changes rarely (intent). `{context_dir}/PROJECT_STATE.md` changes often (progress). Keep that separation.
- **Downstream skills must not reload entire prior narratives.** They consume `{context_dir}/SPEC.md` plus the relevant compact `*.contract.yaml`, not full HLD prose or full chat history.
...
- Marking `{context_dir}/SPEC.md` "approved" — the human owns that.
