...
description: >-
  ...
  artifacts/LEARNINGS.md (append) and optionally a LEARN contract. /review artifacts
  are optional inputs.
---
...
Turn a **completed** task (status `done` on the tasks contract) into **compact, reusable knowledge**. Keeps **`artifacts/`** as the durable source of truth.
...
- **Required (complete):** matching task row `status: done` on `artifacts/<FEATURE>_TASKS.contract.yaml` (legacy plan `done` acceptable).
- **Optional:** `artifacts/<FEATURE>_C<n>_REVIEW.contract.yaml`.
- **Optional:** `artifacts/SPEC.md` if lessons propose process changes (list sections; do not silently rewrite SPEC).
...
4. **Append** `artifacts/LEARNINGS.md` (`## <YYYY-MM-DD> — <TASK_ID>`).
5. **Write** `artifacts/<FEATURE>_C<n>_LEARN.contract.yaml`.
...
| Path | Change | Notes |
|------|--------|-------|
| `artifacts/LEARNINGS.md` | Created or appended | Dated section |
...
tasks_contract_path: "artifacts/<FEATURE>_TASKS.contract.yaml"
